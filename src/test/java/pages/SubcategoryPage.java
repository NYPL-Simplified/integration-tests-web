...
public class SubcategoryPage extends Form {
    ...
    public List<String> getBookTitles() {
        return getListOfTextValues(getListOfElements(BOOK_TITLES_XPATH_LOCATOR));
    }
    ...
    public void cancelReservationForBook(BookInfo bookInfo) {
        ...
        getElementFactory().getButton(By.xpath(String.format(ACTION_BUTTON_OF_BOOK_WITH_GIVEN_TITLE_XPATH_LOCATOR_PATTERN, title, BookActionButtons.CANCEL_RESERVATION.getAction())), title).click();
    }

    public boolean isBookPresent(BookInfo bookInfo) {
        return getBookNameButton(bookInfo).state().isDisplayed();
    }

    public void openBookWithGivenActionButton(BookActionButtons bookActionButtons) {
        IButton btnOpenBookWithGivenButton = getElementFactory().getButton(By.xpath(String.format(BOOK_WITH_GIVEN_BUTTON, bookActionButtons.getAction())), "View book details");
